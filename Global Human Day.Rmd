...
rwa_new = leveled %>% filter(country_iso3 == 'RWA') %>% group_by(Category) %>% mutate(hoursPerDayCombined = sum(hoursPerDayCombined)) %>% 
  ungroup() %>% 
  mutate(Category = fct_reorder(Category,hoursPerDayCombined))

# Creating the comparative chart
leveled %>% mutate(Category = ordered(Category,levels(rwa_new$Category))) %>%  ggplot(aes(hoursPerDayCombined,Category,fill = country_iso3))+
  geom_col()+
